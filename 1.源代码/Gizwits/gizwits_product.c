...
int8_t gizwitsEventProcess(eventInfo_t *info, uint8_t *gizdata, uint32_t len)
{
    ...
    for(i=0; i<info->num; i++)
    {
        ...
        case EVENT_FAN:
            ...
            break;
        case EVENT_JSQ:
            currentDataPoint.valueJSQ = dataPointPtr->valueJSQ;
            GIZWITS_LOG("Evt: EVENT_JSQ %d \n", currentDataPoint.valueJSQ);
            if(0x01 == currentDataPoint.valueJSQ)
            {
            //user handle
							count_m = 3;
							KeyNum = 3;
...
void userHandle(void)
{
    	...
    	currentDataPoint.valuesmoge = sensorData.Smoge;//Add Sensor Data Collection
			...
			currentDataPoint.valueJSQ = driveData.Jsq_Flag;
