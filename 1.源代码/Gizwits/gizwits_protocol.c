...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    if(0x01 == aFlag->flagLed)
    {
        bitFieldBitLen += Led_LEN;
    }
    if(0x01 == aFlag->flagFAN)
    {
        bitFieldBitLen += FAN_LEN;
    }
    if(0x01 == aFlag->flagJSQ)
    {
        bitFieldBitLen += JSQ_LEN;
    }
    if(0x01 == aFlag->flagcontrol_mode)
    {
        bitFieldBitLen += control_mode_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagLed)
    {
        info->event[info->num] = EVENT_Led;
        info->num++;
        dataPoints->valueLed = gizVarlenDecompressionValue(bitFieldOffset,Led_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagJSQ)
    {
        info->event[info->num] = EVENT_JSQ;
        info->num++;
        dataPoints->valueJSQ = gizVarlenDecompressionValue(bitFieldOffset,JSQ_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += JSQ_LEN;
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagcontrol_mode)
    {
        info->event[info->num] = EVENT_control_mode;
        info->num++;
        dataPoints->valuecontrol_mode = gizVarlenDecompressionValue(bitFieldOffset,control_mode_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += control_mode_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueJSQ != cur->valueJSQ)
    {
        GIZWITS_LOG("valueJSQ Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagJSQ = 1;
        ret = 1;
    }
    if(last->valuecontrol_mode != cur->valuecontrol_mode)
    {
        GIZWITS_LOG("valuecontrol_mode Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagcontrol_mode = 1;
        ret = 1;
    }
    ...
    if(last->valuesmoge != cur->valuesmoge)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuesmoge Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagsmoge = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagJSQ)
    {
        gizVarlenCompressValue(bitFieldOffset,JSQ_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueJSQ);
        bitFieldOffset += JSQ_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagcontrol_mode)
    {
        if(dataPoints->valuecontrol_mode >= control_mode_VALUE_MAX)
        {
            GIZWITS_LOG("[ERROR] valuecontrol_mode Error , Illegal Overstep\n");
            return -1;
        }
        else
        {
                gizVarlenCompressValue(bitFieldOffset,control_mode_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuecontrol_mode);
                bitFieldOffset += control_mode_LEN;
        }
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagsmoge)
    {
        devStatusTmp.valuesmoge = exchangeBytes(gizY2X(smoge_RATIO,  smoge_ADDITION, dataPoints->valuesmoge)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuesmoge,smoge_LEN);
        byteFieldOffset += smoge_LEN;
    }
